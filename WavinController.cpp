...
WavinController::WavinController(const struct wc_io_descriptor *io)
{
  struct wc_io_descriptor io = *io;
  io.init();
}
...
bool WavinController::recieve(uint16_t *reply, uint8_t cmdtype)
{
  ...
  {
    n += io.read(buffer, RECIEVE_BUFFER_SIZE - n);
...
{
  // TODO: Empty recieve buffer before sending
  uint16_t written = io.write(data, lenght);
  /*
  if (written != length)
    FAIL
  */
